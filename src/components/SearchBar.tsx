...
export const SearchBar: React.FC<SearchBarProps> = ({ onLocationSelect, placeholder, isDark, currentLanguage }) => {
  ...
  const handleVoiceCityDetected = async (cityName: string) => {
    console.log('Voice search detected city:', cityName);
    setQuery(cityName);
    ...
    try {
      ...
      if (cities.length > 0) {
        // Automatically select the first result for better UX
        const firstResult = cities[0];
        setQuery(`${firstResult.name}, ${firstResult.country}`);
        onLocationSelect(firstResult.lat, firstResult.lon);
      } else {
